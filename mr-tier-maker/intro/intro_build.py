...
def plan4(src, n, out):
    ...
    lab_w=104; rw=800; gap=9; rh=112; bx=800
    ry=(1080-(6*rh+5*gap))//2
    b=[]
    for i,(l,c,ink) in enumerate(G.TIERS):
        y=ry+i*(rh+gap)
        b.append(f"drawbox=x={bx}:y={y}:w={lab_w}:h={rh}:color={c}:t=fill")
        b.append(f"drawtext=fontfile={MONO}:text='{l}':fontcolor={ink}:fontsize=72:"
                 f"x={bx}+({lab_w}-text_w)/2:y={y}+({rh}-text_h)/2-5")
        b.append(f"drawbox=x={bx+lab_w}:y={y}:w={rw}:h={rh}:color=0x101A2E@0.82:t=fill")
    fc=(f"[1:v]{','.join(b)}[bg];"
